...
bot.on("message", async(message) =>{
  ...
  if(Date.now() >= bot.db.event.deadline && bot.db.event.running){
    ...
    leaderboard = leaderboard.filter(user => !user.profile.badge.badges.includes("<:sq_11:837843829833990235>") && user.profile.gamesPlayed >= 1)
    ...
    const congMsg = await adChannel.send(`> **Event has Ended!**\n> **Winners:** ${leaderboard.map(user => { return `${bot.users.cache.get(user.userID) ? bot.users.cache.get(user.userID) : `Unknown user`}` }).join(", ")}`, {
      embed:{
        ...
        fields:[
          {name: `Winners`, value: `${leaderboard.map(user => { return `**${bot.users.cache.get(user.userID) ? bot.users.cache.get(user.userID) : `Unknown user`}** : ${user.profile.badge.badges.includes("<:sq_11:837843829833990235>") ? `They have claimed their badge! Hope they like it` : `They haven't claimed their badge yet.. Let's wait until they claim it`}` })}`}
        ],
      ...
      if(leaderboard[i]){
        ...
        const msg = await member.send(`> **Congratulations!**`, {
          embed:{
            ...
            fields:[
              {name: `Statistics`, value: `> **Final rank:** #${i+1}\n> **Prize:** You have received an unique and speciall badge in your profile of Squarez. This is how the badge looks like: <:sq_11:837843829833990235>\n> Enjoy it!`}
            ],
            color: bot.config.embed_color,
            timestamp: Date.now(),
            footer: { text: bot.user.username }
        ...
        collector.on("collect", async(button) =>{
            ...
            if(button.id === "accept-badge-button"){
              ...
              if(player.profile.badge.badges.includes("<:sq_11:837843829833990235>")) return button.channel.send(`> **Oops!**`, {
                embed:{
              ...
              player.profile.badge.badges.push("<:sq_11:837843829833990235>")
